...
<div class="tab-pane-active" id="receipt">
   <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example" width="100%">
    ...
    <tbody>
          ...
          <?php $i=1256;?>
            <?php foreach ($getall_review as $key): 
            $id =$key['id'];
            $pro_id =$key['pro_id'];
            $userid =$key['userid'];
            $status =$key['status'];
            $status_fr_index =$key['status_fr_index'];
            ?>
                <tr>
                <td id="id_<?php echo $id;?>"><?php echo $key['id']; ?></td>
                <td id="pro_id_<?php echo $id;?>"><?php echo $key['pro_id']; ?></td>
                <td id="userid_<?php echo $id;?>"><?php echo $key['userid']; ?></td>
                <td><?php echo $key['mem_review']; ?></td>
              ...
              <td><input type="checkbox" name="chk" id="approved_review_fr_index<?php echo $id;?>" <?php if($status_fr_index=="1"){?>checked<?php } ?> ></td>
                    <?php $i=$i+1;?>
                </tr>
              <script type="text/javascript">
              $(document).ready(function() {
                $("#approved_review_"+<?php echo $id;?>).click(function(e){
                  ...
                  //alert(id);
                  $.ajax({
                  url: "<?php echo base_url(); ?>" + "index.php/property/approved_review",
                  type: "POST",
                  data: {id:id},
