...
echo $msg;
 ?>
<?php //$topic_id=$getspecific_topic_with_comments_by_id['0']['topic_id'];
      //$pro_id=$getspecific_topic_with_comments_by_id['0']['pro_id'];
    

?><?php //echo site_url('property/property_images_upload/'.$pro_id.'/'.$dev_id);?>
 <form id="insert_comment" action="<?php //echo site_url('group_discussion/add_comment/'.$topic_id.'/'.$pro_id);?>" method="post">
                 <div class="form-group">
                    <label for="exampleInputPassword1">Topic</label>
                      <input type="text" class="form-control" disabled name="topic" value="<?php //echo getTopicname($topic_id);?>"
                          id="topic" />
                          <input type="hidden" class="form-control" name="topic_id" value="<?php //echo $topic_id;?>"
                          id="topic_id" />
                   ...
                   <textarea class="form-control" rows="3" cols="8"  name="comment" id="comment"></textarea><?php echo form_error('address1_dev', '<div class="error" style="color:red;">', '</div>');?> 
                  </div>
                  <button type="submit_join_group" class="btn btn-primary" id="submit_join_group">Submit comment</button>
</form>
<!-- <ol id="update" class="timeline">
<div id="list" style="border-width:4px; border-style:solid; height:200px;">
	
   <?php foreach ($getspecific_topic_with_comments_by_id as $value) {?>
<?php echo $value['user_id'].':-  '.$value['comment'];?><br>
<?php } ?>
...
<li class="box">
<?php foreach ($getspecific_topic_with_comments_by_id as $value) {
	if ($value['email_member']==$userid){ ?>
     <div align="right" style="padding-right:20px;">
<img src="<?php echo base_url($value['member_image']);?>" class="com_img">

 <span class="com_name"><b><?php echo $value['email_member'].':- </b> '.$value['comment'];?></span> 
</div><br />
	<?php  } else{ ?>
